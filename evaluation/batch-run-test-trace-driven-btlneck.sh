...
declare -a serverBtlneck=(300 500)
declare simTime=(60)
...
export filename_prefix="result_trace_driven_btlneck"
...
# 结果分析
csv_file="results/result_trace_driven_btlneck.csv"

if [ -f "$csv_file" ]; then
  echo ""
  echo "==== result_trace_driven_bltneck.csv 分析 ===="
  # 获取所有不同的btlneck值
  btlnecks=$(awk -F',' 'NR>1{print $5}' "$csv_file" | sort -n | uniq)
  for btl in $btlnecks; do
    echo ""
    echo "btlneck=($btl)时："
    # Vanilla (policy=0)
    vanilla_line=$(awk -F',' -v btl="$btl" '$1==0 && $5==btl {print $0}' "$csv_file" | head -n 1)
    ...
    # Plum (policy=1)
    plum_line=$(awk -F',' -v btl="$btl" '$1==1 && $5==btl {print $0}' "$csv_file" | head -n 1)
    if [ -n "$plum_line" ]; then
