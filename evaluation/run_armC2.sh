...
run_c2() {
    seed=$1
    ns3_output=$(NS_GLOBAL_VALUE="RngRun=$seed" ./ns3 run "scratch/test_wifi_channel --mode=sfu --logLevel=0 --simTime=${SIMT} --policy=2 --nClient=${N} --qoeType=${QOET} --mlpred=0 --green=1 --obsdeliv=${OBS}" 2>&1)
    ...
    echo "C2, 2, 1, $OBS, $N, $seed, $QOET, $avg_thp, $min_thp, $tail_thp, $qoe, $avg_rtt, $rtt90" >> $OUT
    echo "done C2 seed=$seed"
}
export -f run_c2
cd $NS3_DIR
pkill -9 -f "solver.py" 2>/dev/null || true
sleep 2
GREEN_CFG=${CURRENT_DIR}/../scripts/solver/green_cfg_n3v2.json python3 ${CURRENT_DIR}/../scripts/solver/solver.py -n $N > ${CURRENT_DIR}/solver_armC2.log 2>&1 &
sleep 3
parallel --line-buffer -j${CORE_COUNT} run_c2 ::: "${seeds[@]}"
pkill -9 -f "solver.py" 2>/dev/null || true
echo "C2DONE"
