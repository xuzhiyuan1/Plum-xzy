...
declare -a datasets=(0 1) #0 for TR_GAME, 1 for TR_RESTAURANT

export baseline_policy=0
export filename_prefix="result_trace_driven_dataset"
...
# 结果分析
csv_file="results/result_trace_driven_dataset.csv"
if [ -f "$csv_file" ]; then
  echo ""
  echo "==== result_trace_driven_dataset.csv 分析 ===="
  # 获取所有不同的dataset值
  datasets=$(awk -F',' 'NR>1{print $4}' "$csv_file" | sort -n | uniq)
  for ds in $datasets; do
    echo ""
    echo "dataset=($ds)时："
    # Vanilla (policy=0)
    vanilla_line=$(awk -F',' -v ds="$ds" '$1==0 && $4==ds {print $0}' "$csv_file" | head -n 1)
    ...
    # Plum (policy=1)
    plum_line=$(awk -F',' -v ds="$ds" '$1==1 && $4==ds {print $0}' "$csv_file" | head -n 1)
    if [ -n "$plum_line" ]; then
